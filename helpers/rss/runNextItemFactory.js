...
const fileTypesRegex =
  /(\.(mp3|m4a|aac|mp4|m4p|m4r|3gp|ogg|oga|wma|raw|wav|flac|m4v))/;
...
const runNextItemFactory = async (options) => {
  const {
    items,
    policy: { downloadRoot, fetchAllItems, ignoreByGuid, oldestDownload } = {},
    title,
  } = options;
  ...
  const runNextItem = () => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (guidAddedFileExists) {
          const { foundSizeInBytes: newSizeInBytes } = checkIfEnclosureExists({
            destinationFilepath: guidIncludedDestinationFilename,
            length,
          });
          // NOTE: we save this filesize here for any future checks
          await writeLocalData(controlFileFilename, newSizeInBytes.toString());
        }
        resolve(true);
      } catch (error) {
